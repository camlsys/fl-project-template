...
def _partition_data(
    ...
    partition_size = int(len(cast(Sized, trainset)) / num_clients)
    ...
    if iid:
        ...
        if power_law:
            ...
            sorted_data = Subset(trainset, cast(Sequence[int], idxs))
            tmp = []
            for idx in range(num_clients * 2):
                tmp.append(
                    Subset(
                        ...
                        cast(
                            Sequence[int],
                            np.arange(shard_size * idx, shard_size * (idx + 1)),
                        ),
                    )
                )
...
def _balance_classes(
    ...
    idxs = trainset.targets.argsort()
    tmp = [Subset(trainset, cast(Sequence[int], idxs[: int(smallest)]))]
    tmp_targets = [trainset.targets[idxs[: int(smallest)]]]
    for count in np.cumsum(class_counts):
        tmp.append(
            Subset(
                trainset, cast(Sequence[int], idxs[int(count) : int(count + smallest)])
            )
        )
    ...
    shuffled = cast(MNIST, Subset(unshuffled, cast(Sequence[int], shuffled_idxs)))
...
def _sort_by_class(
    ...
    for count in np.cumsum(class_counts):
        tmp.append(
            Subset(trainset, cast(Sequence[int], idxs[start : int(count + start)]))
        )  # add rest of classes
        tmp_targets.append(trainset.targets[idxs[start : int(count + start)]])
        start += count
    sorted_dataset = cast(MNIST, ConcatDataset(tmp))  # concat dataset
